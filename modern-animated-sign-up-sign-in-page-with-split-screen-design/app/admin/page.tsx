...
import { ArrowLeft, Check, Loader2, Shield, Trash2, X, ShoppingBag, Users } from "lucide-react"
import { useCallback, useEffect, useState } from "react"
import { useAuth } from "@/lib/auth/auth-context"
import { fetchPendingProfiles, updateProfileStatus } from "@/lib/auth/admin-service"
import { getProducts, adminDeleteProduct } from "@/lib/marketplace/product-service"
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"users" | "marketplace">("users")
  ...
  return (
    <div className="min-h-screen bg-[#1a1a24] px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/50">Review registrations and moderate marketplace listings</p>
            </div>
          </div>
          <Link
            href="/marketplace"
        ...
        {/* Tab Navigation */}
        <div className="mb-6 flex gap-3 border-b border-white/5 pb-3">
          <button
            type="button"
          ...
          {isLoadingData ? (
            ...
            <div>
              <div className="border-b border-white/5 px-6 py-4">
                ...
                <p className="mt-1 text-xs text-white/40">
                  Manage and delete active items across the campus marketplace
                </p>
              </div>

              {listings.length === 0 ? (
                <div className="px-6 py-12 text-center">
                  <p className="text-sm text-white/50">No active marketplace listings.</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
